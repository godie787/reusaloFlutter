...
class LibrosCatScreen extends StatefulWidget {
  const LibrosCatScreen({super.key});

  @override
  State<LibrosCatScreen> createState() => _LibrosCatScreenState();
}

class _LibrosCatScreenState extends State<LibrosCatScreen> {
  List<dynamic>? listaProductosLibros;
  ...
  void initState() {
    super.initState();
    _obtenerProductosLibros();
  }

  Future<void> _obtenerProductosLibros() async {
    try {
      final productos = await productosProvider.getProductos();
      setState(() {
        listaProductosLibros = productos
            .where((producto) => producto['id_categoria'] == 10)
            .toList();
        productosFiltrados = List.from(listaProductosLibros!);
      });
    } catch (error) {
      // Manejo de errores
  ...
  void _aplicarFiltros(String searchQuery, RangeValues priceRange) {
    if (listaProductosLibros == null) return;

    setState(() {
      productosFiltrados = listaProductosLibros!.where((producto) {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              if (listaProductosLibros != null &&
                  listaProductosLibros!.isNotEmpty)
                Expanded(
