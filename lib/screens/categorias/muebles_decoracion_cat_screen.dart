...
class MueblesDecoracionCatScreen extends StatefulWidget {
  const MueblesDecoracionCatScreen({super.key});

  @override
  State<MueblesDecoracionCatScreen> createState() =>
      _MueblesDecoracionCatScreenState();
}

class _MueblesDecoracionCatScreenState
    extends State<MueblesDecoracionCatScreen> {
  List<dynamic>? listaProductosMueblesAccesorios; // Cambiado de Map a List
  ...
  void initState() {
    super.initState();
    _obtenerProductosMueblesDecoracion();
  }

  Future<void> _obtenerProductosMueblesDecoracion() async {
    try {
      final productos = await productosProvider.getProductos();
      setState(() {
        listaProductosMueblesAccesorios = productos
            .where((producto) => producto['id_categoria'] == 9)
            .toList();
        productosFiltrados = List.from(listaProductosMueblesAccesorios!);
      });
    } catch (error) {
      // Manejo de errores
  ...
  void _aplicarFiltros(String searchQuery, RangeValues priceRange) {
    if (listaProductosMueblesAccesorios == null) return;

    setState(() {
      productosFiltrados = listaProductosMueblesAccesorios!.where((producto) {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              if (listaProductosMueblesAccesorios != null &&
                  listaProductosMueblesAccesorios!.isNotEmpty)
                Expanded(
